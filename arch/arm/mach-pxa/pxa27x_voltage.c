 ...
 *
 * History:
 * 2006-02-04 : Michal Sroczynski <msroczyn-ZGDBLhmm9VR4rM3dGMyr8Q@xxxxxxxxxx>
...
#define dbg(msg...) \
	do { \
		printk(KERN_DEBUG "pxa27x_voltage:%s: ", __func__); \
		printk(msg); \
	} while (0);
#else
#define dbg(msg...)
...
#define MAX_VOLTAGE_DROP \
	(freq2mv_table[ARRAY_SIZE(freq2mv_table)-1].mv - freq2mv_table[0].mv)
...
retry:									\n\
	...
	mcreq	p14, 0, r0, c7, c0, 0	@ write to PWRMODE on 2nd pass only\n\
									\n\
	@ Read VC bit until it is 0, indicates that the VoltageChange   \n\
	@ is done. On first pass, we never set the VC bit, so it will   \n\
	@ be clear already.						\n\
VoltageChange_loop:							\n\
		     ...
		     struct pxa27x_voltage_chip *chip)
{
	...
	if (sign > 0 && (old_mv + chip->step*steps < new_mv))
		steps++;
	else if (!steps)
		return 0;
			  ...
			  struct pxa27x_voltage_chip *chip)
{
	...
	if (chip->step) {
		...
		PCMD(0) = chip->mv2cmd(freq2mv(f->new)) | PCMD_LC;
		tmp = PCMD(0);
		dbg("one command voltage change : PVCR=%x, PCMD(0)=%x\n",
		    PVCR, tmp);
	}
			      ...
			      void *data)
{
	int ret = 0;
	struct pxa27x_voltage_chip *chip =
		container_of(nb, struct pxa27x_voltage_chip, freq_transition);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Michal Sroczynski <msroczyn-ZGDBLhmm9VR4rM3dGMyr8Q@xxxxxx>, "
