...
static void
recon_pxa_ll_pm_suspend(unsigned long unused)
{
    struct suspres *sp;

    // printk( "recon_pxa_ll_pm_suspend\n" );
...
static struct pxa2xx_udc_mach_info udc_mach_info = {
    .gpio_vbus          = RECON_GPIO_USB_ON,
    .gpio_pullup        = RECON_GPIO_USB_SS,
};
