...
static int
recon_keys_suspend(struct platform_device *pdev, pm_message_t state)
{ /* (= */
    if(isr_state == PB_DOWN_POWER_OFF) {
        /* Wait until release allowing for soft reset */
...
#define recon_keys_suspend NULL
...
static int
recon_keys_probe( struct platform_device *pdev )
{
    int i;
    int retval;
...
static int
recon_keys_remove( struct platform_device *pdev )
{
...
static struct platform_driver recon_keys_driver = {
    .driver = {
            .name = "recon-keys",
    },
    .probe      = recon_keys_probe,
    .remove     = recon_keys_remove,
    .suspend    = recon_keys_suspend,
    .resume     = NULL,
};
...
{
    return platform_driver_register( &recon_keys_driver );
}

static void __exit
recon_keys_exit( void )
{
    platform_driver_unregister( &recon_keys_driver );
}
