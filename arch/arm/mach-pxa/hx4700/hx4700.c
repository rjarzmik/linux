...
static void hx4700_irda_transceiver_mode(struct device *dev, int mode)
{
	...
	if (mode & IR_OFF)
		gpio_set_value(GPIO_NR_HX4700_IR_ON_N, 1);
	else
		gpio_set_value(GPIO_NR_HX4700_IR_ON_N, 0);
...
struct platform_device hx4700_egpio = {
	.name = "htc-egpio",
	.id   = -1,
	.dev  = {
...
static void __init hx4700_init_irq(void)
{
	...
	set_irq_chained_handler(gpio_to_irq(0), hx4700_irq_handler);
	set_irq_type(gpio_to_irq(0), IRQT_FALLING);
#endif
}
...
static struct resource asic3_resources[] = {
	...
	[1] = {
		.start	= gpio_to_irq(GPIO_NR_HX4700_ASIC3_EXT_INT),
		.end	= gpio_to_irq(GPIO_NR_HX4700_ASIC3_EXT_INT),
	...
	[3] = {
		.start	= gpio_to_irq(GPIO_NR_HX4700_ASIC3_SDIO_INT_N),
		.end	= gpio_to_irq(GPIO_NR_HX4700_ASIC3_SDIO_INT_N),
		.flags	= IORESOURCE_IRQ,
	},
};
...
static void __init hx4700_init( void )
{
	...
	gpio_set_value(GPIO_NR_HX4700_ASIC3_RESET_N, 0);
	mdelay(10);
	gpio_set_value(GPIO_NR_HX4700_ASIC3_RESET_N, 1);
	mdelay(10);
	gpio_set_value(GPIO_NR_HX4700_EUART_RESET, 1);
	...
	gpio_set_value(GPIO_NR_HX4700_RS232_ON, 1);
