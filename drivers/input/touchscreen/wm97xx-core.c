 ...
 *                   Added bus interface.
 *     9th Nov 2007  Ian Molton <spyro@f2s.com>
 *                   * Fixed printing of wm9705 codec name.
 *                   * Made codec selection a run-time option as well as build
 *                     time selectable.
 */
...
static int wm97xx_probe(struct device *dev)
{
	...
	if ((id = wm97xx_reg_read(wm, AC97_VENDOR_ID1)) != WM97XX_ID1) {
		err("could not find a wm97xx, found a %04x instead\n", id);
		kfree(wm);
		return -ENODEV;
	}
	...
	switch(wm->id & 0xff) {
		...
		case 0x05: wm->codec = &wm9705_codec; break;
#endif
#ifdef CONFIG_TOUCHSCREEN_WM9712
		case 0x12: wm->codec = &wm9712_codec; break;
#endif
#ifdef CONFIG_TOUCHSCREEN_WM9713
		case 0x13: wm->codec = &wm9713_codec; break;
#endif
		default:
			err("Specifc support for wm97%02x not compiled in.",
			    wm->id & 0xff);
			kfree(wm);
			return -ENODEV;
	}
