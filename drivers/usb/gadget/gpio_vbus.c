 ...
 * Needs to be probed before the UDC driver that will use it.
 *
...
int gpio_vbus_set_peripheral(struct otg_transceiver *otg,
...
int gpio_vbus_set_power(struct otg_transceiver *otg, unsigned mA)
{
	struct gpio_vbus_data *gpio_vbus;
...
int gpio_vbus_set_suspend(struct otg_transceiver *otg, int suspend)
{
	/* draw max 0 mA from vbus in suspend mode */
	return gpio_vbus_set_power(otg, 0);
}
...
static int __init gpio_vbus_probe(struct platform_device *pdev)
{
	...
	err = request_irq(irq, gpio_vbus_irq, get_irq_flags(res),
		"vbus_detect", pdev);
	if (err) {
