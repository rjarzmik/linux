...
#ifdef CONFIG_MACH_RECON
static int recon_record_copy(struct snd_pcm_substream *substream, int channel,
    snd_pcm_uframes_t pos, void __user *buf, snd_pcm_uframes_t count)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	int bytes = frames_to_bytes(substream->runtime, count);
	char *hwbuf = runtime->dma_area + frames_to_bytes(substream->runtime, pos);
	int cpbytes = bytes;
	signed short *src;

	src = (signed short *)hwbuf;
	...
	if (copy_to_user(buf, hwbuf, cpbytes))
		return -EFAULT;
	return 0;
}

static struct snd_pcm_ops recon_record_sound_ops = {
	.open		= snd_pxa2xx_i2sound_pcm_open,
	.close		= snd_pxa2xx_i2sound_pcm_close,
	.ioctl		= snd_pcm_lib_ioctl,
	.hw_params	= snd_pxa2xx_i2sound_pcm_hw_params,
...
#ifdef CONFIG_MACH_RECON
		if(s_id == SNDRV_PCM_STREAM_CAPTURE) {
			snd_pcm_set_ops(card.apcm, s->id, &recon_record_sound_ops);
		} else {
