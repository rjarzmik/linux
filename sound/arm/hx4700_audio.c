...
{
	gpio_set_value(GPIO_NR_HX4700_CODEC_ON, mode);
}

static void snd_hx4700_audio_set_codec_reset(int mode)
{
	gpio_set_value(GPIO_NR_HX4700_CODEC_PDN_N, !mode);
}

static void snd_hx4700_audio_set_headphone_power(int mode)
{
	gpio_set_value(GPIO_NR_HX4700_HP_DRIVER, mode);
}

static void snd_hx4700_audio_set_speaker_power(int mode)
{
	gpio_set_value(GPIO_NR_HX4700_SPK_SD_N, mode);
}

static inline int snd_hx4700_audio_hp_detect(void)
{
	return !gpio_get_value(GPIO_NR_HX4700_EARPHONE_DET_N);
}
...
static void snd_hx4700_audio_hp_detection_on(void)
{
	...
	irq = gpio_to_irq(GPIO_NR_HX4700_EARPHONE_DET_N);
...
{
	free_irq(gpio_to_irq(GPIO_NR_HX4700_EARPHONE_DET_N), NULL);
}
